#Morgan Rault - Paper Piano
import RPi.GPIO as GPIO
from time import sleep, time
import pygame
from array import array

#setup the pins and frequencies for the notes(C, E, G, B)
keys = [20, 16, 12, 26]
freqs = [261.6, 329.6, 392.0, 493.9]
notes = []
#setup the button pins
play = 19
record = 21
#setup the LED pins
red = 27
green = 18
blue = 17 #only use if red is too dim

MIXER_FREQ = 44100
MIXER_SIZE = -16
MIXER_CHANS = 1
MIXER_BUFF = 1024

#the note generator class
class Note(pygame.mixer.Sound):
    #note that volume ranges from 0.0 to 0.1
    def __init__(self, frequency, volume):
        self.frequency = frequency
        #initialize the note using an array of samples
        pygame.mixer.Sound.__init__(self, buffer=self.build_samples())
        self.set_volume(volume)
    #build an array of samples for the current note
    def build_samples(self):
        #calculate the period of amplitude of the notes wave
        period = int(round(MIXER_FREQ / self.frequency))
        amplitude = 2 ** (abs(MIXER_SIZE) -1) -1
        #initialize the notes samples (using an array of signed 16-bit "shorts")
        samples = array("h", [0] * period)
        
        #work on this#
        #generate the notes samples
        for t in range(period):
            if (t < period / 2):
                samples[t] = amplitude
            else:
                samples[t] = -amplitude
        return samples

#waits until the note is pressed
def wait_for_note_start():
    while (True):
        #first check for notes
        for key in range(len(keys)):
            if (GPIO.input(keys[key])):
                return key
        #next check for the play button
        if (GPIO.input(play)):
            #debounce the switch
            print ("Playing Back...")
            while(GPIO.input(play)):
                sleep(0.01)
            return "play"
        #finally check for the record button
        if (GPIO.input(record)):
            #debounce the switch
            print ("Recording...")
            while(GPIO.input(record)):
                sleep(0.01)
            return "record"
        sleep(0.01)

#waits until the note is released
def wait_for_note_stop(key):
    while(GPIO.input(key)):
        sleep(0.1)

#plays a recorded song
def play_song():
    #each element in the song list is a list of composed parts
    for part in song:
        note, duration = part
        #if it's a scilence, delay for it's duration
        if (note == "SILENCE"):
            sleep(duration)
            #otherwise, play the note for it's duration
        else:
            notes[note].play(-1)
            sleep(duration)
            notes[note].stop()

#initlize the mixer; then initialize the pygame library
pygame.mixer.pre_init(MIXER_FREQ, MIXER_SIZE, MIXER_CHANS, MIXER_BUFF)
pygame.init()

#use the Broadcom pin mode
GPIO.setmode(GPIO.BCM)

#setup the input pins
GPIO.setup(keys, GPIO.IN, GPIO.PUD_DOWN)
GPIO.setup(play, GPIO.IN, GPIO.PUD_DOWN)
GPIO.setup(record, GPIO.IN, GPIO.PUD_DOWN)

#setup the output pins
GPIO.setup(red, GPIO.OUT)
GPIO.setup(green, GPIO.OUT)
GPIO.setup(blue, GPIO.OUT)

for freq in freqs:
    notes.append(Note(freq, 1))

#begin in a non recording state and initalize the song
recording = False
song = []

print("Welcome to Paper Piano")
print("Press Ctrl + C to exit ...")

try:
    while (True):
        #start a timer
        start = time()
        #play a note when  pressed
        #also detect play/record
        key = wait_for_note_start()
        #note the duration of scilence
        duration = time() - start
        # if recording append the duration of the scilence
        if (recording):
            song.append(["SILENCE", duration])
           #if record was pressed 
        if (key == "record"):
            if (not recording):
                song = []
            recording = not recording
            GPIO.output(red, recording)
        #if the button was pressed    
        elif (key == "play"):
            #if recording stop
            if (recording):
                recording = False
                GPIO.output(red, False)
            GPIO.output(green, True)
            play_song()
            GPIO.output(green, False)
        #if the button was pressed
        elif (key == "play"):
            #if recording stop
            if (recording):
                recording = False
                GPIO.output(red, False)
                #turn on the green led
            GPIO.output(green, True)
            play_song()
            GPIO.output(green, False)
        #otherwise a piano key was pressed
        else:
            #start the timer and paly the note
            start = time()
            notes[key].play(-1)
            wait_for_note_stop(keys[key])
            notes[key].stop()
            #once the note is realeased, stop the timer
            duration = time() - start
            #if recording append the note and the duration
            if (recording):
                song.append([key, duration])

except KeyboardInterrupt:
    GPIO.cleanup()